...
Olá, sou **Lorenzo Garcia** — um desenvolvedor Full Stack focado em construir soluções eficientes e seguras para infraestrutura educacional e corporativa. Atualmente, atuo na modernização de processos através de software e no fortalecimento da segurança digital no setor público.

Me especializo no desenvolvimento de aplicações utilizando **Python (Django)** e **React (TypeScript)**. Trabalho ativamente com a implementação de bancos de dados **PostgreSQL** e orquestração de ambientes com **Docker**, focando sempre em escrever código limpo e manter a integridade dos sistemas.
